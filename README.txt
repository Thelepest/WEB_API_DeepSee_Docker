...
TO RUN ON DOCKER, RUN THE FOLLOWING INSTRUCTIONS ON YOUR TERMINAL'S CODE EDITOR.
FOR MAC, ADD "sudo" AFTER EVERY INSTRUCTION : 
docker-compose run store sh -c "python manage.py makemigrations" 
docker-compose run store sh -c "python manage.py migrate"
